...
import com.jxtii.oa.common.beanvalidator.BeanValidators;
...
import com.jxtii.oa.common.utils.excel.ImportExcel;
...
public class UserController extends BaseController {
    ...
    public String importFile(MultipartFile file, RedirectAttributes redirectAttributes) {
        ...
        suffix.add(".xls");
        suffix.add(".xlsx");
        if (fileName.lastIndexOf(".") == -1 || !suffix.contains(fileName.substring(fileName.lastIndexOf("."), fileName.length()))) {
            addMessage(redirectAttributes, "文件格式错误，请上传Excel文件");
            return "redirect:" + adminPath + "/sys/user/list?repage";
        }
        try {
            int successNum = 0;
            ...
            for (User user : list) {
                try {
                    if (!systemService.isExistsUser(user.getLoginName())) {
                        if (user.getRoleList() == null) {
                            List<Role> roleList = Lists.newArrayList();
                            roleList.add(new Role("普通用户"));
                            user.setRoleList(roleList);
                        }
            ...
            addMessage(redirectAttributes, "导入用户失败，请使用模板导入！");
        }
        return "redirect:" + adminPath + "/sys/user/list?repage";
    }
    ...
    public String info(User user, HttpServletResponse response, Model model) {
        ...
        if (StringUtils.isNotBlank(user.getName())) {
            ...
            // fix:积分不刷新BUG
            User user1 = UserUtils.getByLoginName(currentUser.getLoginName());
    ...
    public String modifyPwd(String oldPassword, String newPassword, Model model, HttpServletResponse response) {
