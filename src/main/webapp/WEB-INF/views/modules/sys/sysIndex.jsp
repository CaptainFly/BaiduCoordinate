...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function getMenu(list, tpl, row, i) {
                var s = "";
                if (i == 0) {
                    s = "in";
                }
                $(list).append(Mustache.render(tpl, {row: row , ss : s}));
            }
            ...
            $("#menu a.menu").click(function () {
                ...
                if ($(menuId).length > 0) {
                    ...
                    $.get($(this).attr("data-href"), function (data) {
                        ...
                        var tpl = $("#menuTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g, "");
                        $(".accordion").empty();
                        $(".accordion").attr("id", menuId);
                        for (var i = 0; i < data.length; i++) {
                            getMenu(".accordion", tpl, data[i], i);
                        }
//                        $("#left .accordion").hide();
                    });
                    // 链接去掉虚框
                            ...
                            <li class="menu ${not empty firstMenu && firstMenu ? ' active' : ''}">
                                <c:if test="${empty menu.href}">
                                    <a class="menu" href="javascript:"
                                       data-href="${ctx}/sys/menu/treeJson?parentId=${menu.id}"
    ...
    <div class="container-fluid">
        <div id="content" class="row-fluid">
            ...
            <script type="text/template" id="menuTpl">//<!--
                    ...
                    <div class="accordion-group" id="group-menu-{{row.id}}">
                        <div class="accordion-heading">
                            <a class="accordion-toggle" id="accordion-toggle-{{row.id}}" onclick='getChildMenu("{{row.id}}")' data-toggle="collapse" data-parent="#menu-{{row.parentId}}" id="menu-{{row.id}}"  data-href="#collapse-{{row.id}}" href="#collapse-28" title=""><i class="icon-chevron-right"></i>&nbsp;{{row.name}}</a>
                        </div>
                        <div id="collapse-{{row.id}}" class="accordion-body collapse">
                        <div class="accordion-inner">
                            <ul class="nav nav-list" id="nav-{{row.id}}">
                               </ul>
                             </div>
                        </div>
                    </div>
            ...
            <script>
                function getChildMenu(pid) {
                    if ($("#collapse-" + pid).hasClass("in")) {
                        $("#collapse-" + pid).removeClass("in");
                        $("#accordion-toggle-" + pid).children("i").removeClass('icon-chevron-down').addClass('icon-chevron-right');
                    } else {
                        $.get("${ctx}/sys/menu/treeJson?parentId=" + pid, function (data) {
                            if (data.indexOf("id=\"loginForm\"") != -1) {
                                alert('未登录或登录超时。请重新登录，谢谢！');
                                top.location = "${ctx}";
                                return false;
                            ...
                            $("#nav-" + pid).empty();
                            var tpl = $("#menuChildTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g, "");
                            for (var i = 0; i < data.length; i++) {
                                addChildMenu("#nav-" + pid, tpl, data[i]);
                            }
                            $("#collapse-" + pid).addClass("in");
                            $("#accordion-toggle-" + pid).children("i").removeClass('icon-chevron-right').addClass('icon-chevron-down');
                        });

                    }
                    function addChildMenu(list, tpl, row) {
                        $(list).append(Mustache.render(tpl, {row: row}));
                    }
                }
            </script>
            <script type="text/template" id="menuChildTpl">//<!--
                        <li><a data-href=".menu3-{{row.id}}" href="${ctx}{{row.href}}" target="mainFrame">
                            <i class="icon-folder-open"></i>&nbsp;{{row.name}}</a>
							<ul class="nav nav-list" style="margin:0;padding-right:auto;"></ul>
						</li>
						//-->
            </script>
        ...
        <div id="footer" class="row-fluid">
            Copyright &copy; 2012-${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a
                href="http://www.jxtii.com/">江西电信信息产业有限公司</a> ${fns:getConfig('version')}
        </div>
    </div>
</div>
<script type="text/javascript">
